...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ===== Replit Database API Routes =====

  // Save a recipe to the user's collection
  app.post("/api/recipes", async (req: Request, res: Response) => {
    try {
      const { userId, recipe, imageUrl } = req.body;
      
      if (!userId || !recipe) {
        return res.status(400).json({
          error: "Missing required fields",
          details: "userId and recipe are required"
        });
      }
      
      const result = await saveUserRecipe(userId, recipe, imageUrl);
  ...
  app.get("/api/recipes/:userId", async (req: Request, res: Response) => {
    try {
  ...
  app.delete("/api/recipes/:userId/:recipeId", async (req: Request, res: Response) => {
    try {
      ...
      if (!userId || !recipeId) {
        return res.status(400).json({
          ...
          details: "userId and recipeId parameters are required"
        });
      }
  ...
  app.get("/api/user-profile/:userId", async (req: Request, res: Response) => {
    try {
      ...
      const result = await getUserProfile(userId, email);
  ...
  app.patch("/api/user-profile/:userId", async (req: Request, res: Response) => {
    try {
      const userId = req.params.userId;
